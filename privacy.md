...
- **No data is collected.** The extension does not read, record, or send your
  ...
  information.
- **No analytics or tracking.** There is no telemetry, no third-party analytics,
...
- stays within your browser account (if you have browser sync enabled, your
  browser — not us — may sync it across your own devices),
...
- `storage` — to save your feature settings, as described above.
- Access to `youtube.com`, `x.com` / `twitter.com`, and `linkedin.com` — the only
  sites where it applies changes.

It requests no broad host access (no `<all_urls>`).
